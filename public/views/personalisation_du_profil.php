...
<body>
    <ul>
        <?php foreach ($users as $user) : ?>
            <li><?= $user['name']; ?><button onclick="showForm(<?= htmlspecialchars(json_encode($user)); ?>, 'name')">Modifier</button></li>
            <li><?= $user['surname']; ?><button onclick="showForm(<?= htmlspecialchars(json_encode($user)); ?>, 'surname')">Modifier</button></li>
            <li><?= $user['email']; ?><button onclick="showForm(<?= htmlspecialchars(json_encode($user)); ?>, 'email')">Modifier</button></li>
            <li>
    ...
    <script>
        function showForm(user, field) {
            ...
            if (newNameElement) {
                newNameElement.value = user.name;
            }
            if (newSurnameElement) {
                newSurnameElement.value = user.surname;
            }
            if (newEmailElement) {
                newEmailElement.value = user.email;
            }
